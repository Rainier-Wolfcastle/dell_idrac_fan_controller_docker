...
function calculate_interpolated_fan_speed() {
  ...
  local -r -i RETURN_FAN_SPEED=$(("$LOCAL_DECIMAL_FAN_SPEED" + ( ( "$LOCAL_DECIMAL_HIGH_FAN_SPEED" - "$LOCAL_DECIMAL_FAN_SPEED") * ( ("$highest_CPU_temperature" - "$CPU_TEMPERATURE_THRESHOLD_FOR_FAN_SPEED_INTERPOLATION") / ("$CPU_TEMPERATURE_THRESHOLD" - "$CPU_TEMPERATURE_THRESHOLD_FOR_FAN_SPEED_INTERPOLATION") ) ))
