...
#debug
  ...
  for i in {0..9}; do
    ...
    echo "Inputs"
    echo $LOCAL_DECIMAL_FAN_SPEED "Z" $LOCAL_DECIMAL_HIGH_FAN_SPEED "Z" $highest_CPU_temperature "Z" $CPU_TEMPERATURE_THRESHOLD_FOR_FAN_SPEED_INTERPOLATION "Z" $CPU_TEMPERATURE_THRESHOLD  "Z"
    ...
    echo "FAN SPEED RESULT=" $fan_speed
...
function calculate_interpolated_fan_speed() {
  local -r -i LOCAL_DECIMAL_FAN_SPEED=$1
  local -r -i LOCAL_DECIMAL_HIGH_FAN_SPEED=$2
  local -r -i highest_CPU_temperature=$3
  local -r -i CPU_TEMPERATURE_THRESHOLD_FOR_FAN_SPEED_INTERPOLATION=$4
  local -r -i CPU_TEMPERATURE_THRESHOLD=$5
  #local RETURN_FAN_SPEED
  #local RETURN_FAN_SPEED_A
  #local RETURN_FAN_SPEED_B
  local -r -i RETURN_FAN_SPEED_A=$(( ($LOCAL_DECIMAL_HIGH_FAN_SPEED - $LOCAL_DECIMAL_FAN_SPEED) * ($highest_CPU_temperature - $CPU_TEMPERATURE_THRESHOLD_FOR_FAN_SPEED_INTERPOLATION) ))
  local -r -i RETURN_FAN_SPEED_B=$(( ($CPU_TEMPERATURE_THRESHOLD - $CPU_TEMPERATURE_THRESHOLD_FOR_FAN_SPEED_INTERPOLATION) ))
  ...
  local -r RETURN_FAN_SPEED=$(( LOCAL_DECIMAL_FAN_SPEED + ( RETURN_FAN_SPEED_A / RETURN_FAN_SPEED_B ) ))
  echo $RETURN_FAN_SPEED
  #return $(( $LOCAL_DECIMAL_FAN_SPEED + ( $RETURN_FAN_SPEED_A / $RETURN_FAN_SPEED_B ) ))
}
...
function CPU1_OVERHEATING() {
  [ $CPU1_TEMPERATURE -gt $CPU_TEMPERATURE_THRESHOLD ]
  }
function CPU2_HEATING() {
  #local CPU2_TEMPURATURE_LOCAL=$(max 10 $CPU2_TEMPERATURE)
  #[ ${CPU2_TEMPERATURE_LOCAL} -gt ${CPU_TEMPERATURE_THRESHOLD_FOR_FAN_SPEED_INTERPOLATION} ]
  [ 2 -gt 1 ]
  }
function CPU2_OVERHEATING() {
  #local CPU2_TEMPURATURE_LOCAL=$(max 10 $CPU2_TEMPERATURE)
  #[ ${CPU2_TEMPERATURE} -gt ${CPU_TEMPERATURE_THRESHOLD} ]
  [ 1 -gt 2 ]
  }
